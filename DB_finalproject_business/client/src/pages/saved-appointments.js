...
export const SavedAppointments = () => {
  ...
  useEffect(() => {
    const fetchStores = async () => {
      try {
        const responseStores = await axios.get("http://localhost:3001/appointments/");
  ...
  const handleSelectStore = async (storeId) => {
    ...
    try {
      const responseAppointments = await axios.get("http://localhost:3001/appointments/client_appointments");
